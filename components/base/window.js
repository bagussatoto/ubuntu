...
export class Window extends Component {
    ...
    setDefaultWindowDimenstion = () => {
        if (window.innerWidth < 640) {
            this.setState({ height: 90, width: 90 }, this.resizeBoundries);
        }
        else {
            this.setState({ height: 90, width: 85 }, this.resizeBoundries);
        }
    }
...
export function WindowTopBar(props) {
    return (
        <div className={"relative bg-ub-window-title border-t-2 border-white border-opacity-5 py-1.5 px-3 text-white w-full select-none rounded-b-none"}>
            <div className="flex justify-start md:justify-center sm:justify-center text-sm md:text-lg font-bold">{props.title}</div>
        </div>
    )
}
...
export function WindowEditButtons(props) {
    return (
        <div className="absolute select-none right-0 top-0 mt-2 mr-2 flex justify-center items-center">
